...
from starter_app.services.settings import (
    deactivate_user,
    send_deactivation_confirmation_email,
)
...
def at_start(sender, **kwargs):
    ...
    PeriodicTask.objects.get_or_create(
        ...
        task="starter_app.tasks.remove_past_periodic_tasks",
    )

    PeriodicTask.objects.get_or_create(
        crontab=schedule_2,
        name="Remove users with unverified email",
        task="starter_app.tasks.remove_users_with_unverified_email",
    )
